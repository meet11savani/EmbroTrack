...
interface QualityFormProps {
  editingQuality?: Quality | null;
  onClose: () => void;
  onSaved?: () => void;
}

export function QualityForm({ editingQuality, onClose, onSaved }: QualityFormProps) {
  const { addQuality, updateQuality, qualities } = useApp();
  const isEditing = !!editingQuality;
  ...
  useEffect(() => {
    if (editingQuality) {
      setFormData({
        ...
        designNumber: editingQuality.designNumber,
        defaultRate: String(editingQuality.defaultRate || ''),
        unit: editingQuality.unit,
        notes: editingQuality.notes,
      });
    }
  }, [editingQuality]);
  ...
  const handleSubmit = (e: FormEvent) => {
    e.preventDefault();
    const result = validateQuality(formData, qualities, editingQuality?.id);
    if (!result.isValid) {
      setErrors(result.errors);
      return;
    }
    if (isEditing && editingQuality) {
      updateQuality(editingQuality.id, {
        ...
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-gold flex items-center justify-center">
              {isEditing ? <Save size={16} className="text-white" /> : <Sparkles size={16} className="text-white" />}
            </div>
            <h3 className="text-lg font-bold text-navy">{isEditing ? 'Edit Quality' : 'Add Quality'}</h3>
          </div>
