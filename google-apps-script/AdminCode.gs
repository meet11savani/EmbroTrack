...
const ADMIN_COLS = ['username', 'password', 'name', 'phone', 'email', 'createdAt'];
const USER_COLS = ['username', 'password', 'role', 'name', 'phone', 'email', 'address', 'city', 'state', 'pincode', 'gstNumber', 'companyName', 'notes', 'active', 'createdAt'];
...
function handleLogin(params) {
  ...
  for (let i = 1; i < userData.length; i++) {
    if (String(userData[i][0]).trim() === username && String(userData[i][1]) === password) {
...
function handleCreateUser(params) {
  ...
  sheet.appendRow([username, password, role, name, phone, email, address, city, state, pincode, gstNumber, companyName, notes, active ? 'true' : 'false', new Date().toISOString()]);
...
function handleChangePassword(params) {
  ...
  if (!username || !newPassword) {
    return jsonOut({ success: false, error: 'Username and new password are required' });
  }
...
function setupInitialAdmin() {
  ...
  sheet.appendRow([adminUsername, adminPassword, 'Administrator', '', '', new Date().toISOString()]);
